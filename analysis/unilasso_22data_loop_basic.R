...
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript unilasso_22data_loop_basic.R")
  quit("no")
}
...
dataset_names <- c(
  "airfoil_self-noise", "appliances_energy_prediction", "auto_mpg", "automobile", "beijing_pm2.5",
  "bike_sharing", "combined_cycle_power_plant", "communities_and_crime", "communities_and_crime_unnormalized", "communities_and_crime_unnormalized_(target_2)",
  "communities_and_crime_unnormalized_(target_3)", "communities_and_crime_unnormalized_(target_4)", "communities_and_crime_unnormalized_(target_5)", "communities_and_crime_unnormalized_(target_6)", "communities_and_crime_unnormalized_(target_7)",
  "communities_and_crime_unnormalized_(target_8)", "communities_and_crime_unnormalized_(target_9)", "communities_and_crime_unnormalized_(target_10)", "communities_and_crime_unnormalized_(target_11)", "communities_and_crime_unnormalized_(target_12)",
  "communities_and_crime_unnormalized_(target_13)", "communities_and_crime_unnormalized_(target_14)", "communities_and_crime_unnormalized_(target_15)", "communities_and_crime_unnormalized_(target_16)", "communities_and_crime_unnormalized_(target_17)",
  "communities_and_crime_unnormalized_(target_18)", "concrete_compressive_strength", "daily_demand_forecasting_orders", "facebook_metrics", "forest_fires",
  "infrared_thermography_temperature", "infrared_thermography_temperature_(target_2)", "liver_disorders", "metro_interstate_traffic_volume", "parkinsons_telemonitoring",
  "parkinsons_telemonitoring_(target_2)", "power_consumption_of_tetouan_city", "power_consumption_of_tetouan_city_(target_2)", "power_consumption_of_tetouan_city_(target_3)", "real_estate_valuation",
  "servo", "stock_portfolio_performance", "stock_portfolio_performance_(target_2)", "stock_portfolio_performance_(target_3)", "stock_portfolio_performance_(target_4)",
  "stock_portfolio_performance_(target_5)", "stock_portfolio_performance_(target_6)", "superconductivity_data"
)
...
run_single_split <- function(split_id, X, y, dataset_name) {
  ...
  tryCatch({
    ...
    uni_loo_coefs <<- rep(0, ncol(Xtr))  # fallback to zeros if uniInfo fails
  })
  ...
  tryCatch({
    ...
    split_results$unilasso_loo_true_sign_diff <- sum(sign_beta_coef != sign_uni_loo_coefs)  # number of sign differences
  ...
  tryCatch({
    ...
    sign_beta_coef <- sign(beta_coef[active_index])
    sign_uni_loo_coefs <- sign(uni_loo_coefs[active_index])
    split_results$polish_unilasso_sign_diff <- sum(sign_beta_coef != sign_uni_loo_coefs)  # number of sign differences
  ...
  tryCatch({
    ...
    sign_beta_coef <- sign(beta_coef[active_index])
    sign_uni_loo_coefs <- sign(uni_loo_coefs[active_index])
    split_results$lasso_cv_sign_diff <- sum(sign_beta_coef != sign_uni_loo_coefs)  # number of sign differences
  ...
  tryCatch({
    ...
    split_results$unireg_support <- sum(coef(uniReg_cv, s = "lambda.min")[-1] != 0) # exclude intercept
    ...
    sign_beta_coef <- sign(beta_coef[active_index])
    sign_uni_loo_coefs <- sign(uni_loo_coefs[active_index])
    split_results$unireg_sign_diff <- sum(sign_beta_coef != sign_uni_loo_coefs)  # number of sign differences
  ...
  tryCatch({
    ...
    sign_beta_coef <- sign(beta_coef[active_index])
    sign_uni_loo_coefs <- sign(uni_loo_coefs[active_index])
    split_results$least_squares_sign_diff <- sum(sign_beta_coef != sign_uni_loo_coefs)  # number of sign differences
