...
polish.uniLasso <- function(x,y,family=c("gaussian","binomial","cox"),weights=NULL,...){
    ...
    fit1 = cv.glmnet(x,y,offset=pred0,weights=weights,family=family,...) # we used offset here to fit the second stage lasso
    ## Now we stitch them together
    glmnet.fit1 = fit1$glmnet.fit # second stage lasso 
    glmnet.fit0= fit0$glmnet.fit # first stage unilasso
    index=fit0$index[1,] # indexes lambda.min from first stage (unilasso)
    beta1=glmnet.fit1$beta # matrix of coefficients from second stage lasso
    nc=ncol(beta1) # number of lambda values in second stage
    beta0=glmnet.fit0$beta[,index,drop=FALSE] # coefficients from first stage at lambda.min
    glmnet.fit1$beta <- beta1 + beta0[,rep(1,nc)] # add the first stage coefficients to each column of the second stage coefficients
    if(!is.null(glmnet.fit1$a0)){# Interecpt is present
        glmnet.fit1$a0=glmnet.fit1$a0+glmnet.fit0$a0[index] # add intercepts from second stage and first stage 
    }
    glmnet.fit1$df = apply(glmnet.fit1$beta!=0,2,sum) # glmnet.fit1$df is a vector where each entry is the number of nonzero coefficients (i.e., the number of predictors included in the model) for a particular value of lambda. 
