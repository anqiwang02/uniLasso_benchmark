...
uniLasso <- function(x,y,family=c("gaussian","binomial","cox"),weights=NULL,
    ...
    family=match.arg(family) # Make sure the user’s input matches one; Allow partial matching ("gaus" would match "gaussian").
    if(is.null(info)){ # user did not supply info
        info = uniInfo(x,y,family,weights,loob.nit,loob.eps,loo) # uniinfo computes univariate fits
    }
    else {
    ...
    a0=drop(predict(fit,info$beta0)) # basically gamma_0 in step 3 of the algorithm
    if(offset)fit$offset=TRUE
    fit$beta=fit$beta*outer(info$beta,rep(1,length(a0)))# final coefficients have beta_j (from univariate coef) multiplied by theta_j (calculated from second stage)
    fit$a0=a0
    fit$info = info[c("beta0","beta")] # store only the relevant parts of info (intercepts and slopes of the univariate fits)
    class(fit)=c("uniLasso",class(fit)) # set the class of the fit object to "uniLasso"
    fit$call = this.call
    fit #returns the modified fit object from the function 
    }
