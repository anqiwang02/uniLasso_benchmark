...
# Final version for 22 datasets. Oct 22, 2025
```{r}
### setup####
...
```{R}
# select only the MSE 
plot_combined_long1_mse <- plot_combined_long_with_stability %>% filter(metric == "MSE") %>%
...
p_mse_ordered_normalized <- ggplot(plot_combined_long1_mse, aes(x = id_ordered, y = normalized_mse, color = method)) +
  ...
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, size = 26, family = "Times New Roman", face = "bold"),
    axis.text.y = element_text(size = 26, family = "Times New Roman", face = "bold"),  
    axis.title = element_text(size = 30, family = "Times New Roman", face = "bold"),
...
ggsave("/accounts/grad/aqwang/unilasso/figures_22data_default_lambda/app_p_mse_line_22data_normalized.png", 
       plot = p_mse_ordered_normalized, width = 32, height = 45, dpi = 300)
...
plot_combined_long_first8 <-  plot_combined_long_with_normalized %>%
...
ggsave("/accounts/grad/aqwang/unilasso/figures_22data_default_lambda/app_mse_supp_bootstrap_first8data.png", plot = plot_bootstrap_points_sd8, width = 15, height = 24, dpi = 300)  # Increased width from 12 to 15


# second 8 sets of datasets
plot_combined_long_second8 <-  plot_combined_long_with_normalized %>%
...
ggsave("/accounts/grad/aqwang/unilasso/figures_22data_default_lambda/app_mse_supp_bootstrap_second8data.png", plot = plot_bootstrap_points_sd8_2, width = 12, height = 27, dpi = 300)



# last 6 datasets
plot_combined_long_last6 <-  plot_combined_long_with_normalized %>%
...
plot_pred_int_width_sd <- ggplot(pred_int_width_first12, aes(x = method, y = width, color = method)) +
  geom_jitter(width = 0.25, alpha = 0.4, size = 0.5) +  # Increased width, reduced alpha and size
  geom_errorbar(
  ...
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.2))) +  # Reduced from 0.3 to 0.2
  ...
  theme(
    ...
    strip.text = element_text(size = 20, family = "Times New Roman", face = "bold"),  # Changed from 18 to 20
    strip.placement = "outside",
    panel.spacing.y = unit(0.7, "cm"),
    panel.spacing.x = unit(1.2, "cm"),  # Added horizontal spacing between panels
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
  )
...
ggsave("/accounts/grad/aqwang/unilasso/figures_22data_default_lambda/pred_int_width_bootstrap_12data.png", 
       plot = plot_pred_int_width_sd, width = 16, height = 15, dpi = 300)  # Increased width from 14 to 16
...
ggsave("/accounts/grad/aqwang/unilasso/figures_22data_default_lambda/pred_int_width_bootstrap_last10data.png", 
       plot = plot_pred_int_width_sd_last10, width = 16, height = 15, dpi = 300)  # Increased width from 14 to 16
