# Loop to run uniLasso analysis on all 22 datasets with bootstrap sampling (PARALLELIZED)
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript unilasso_22data_100bootstrap.R")
  quit("no")
}
...
# Define all 22 dataset names. for those with more than one response, i picked the first one 
dataset_names <- c(
...
# Initialize storage for individual interval widths (ADDED)
all_interval_widths <- list()
...
for (i in seq_along(dataset_names)) {
  ...
  # Modified function to compute intervals and return individual widths (MODIFIED)
  compute_intervals_with_widths <- function(pred_matrix, method_name, confidence_level = 0.95) {
    ...
    # If no valid points, return NA for all metrics
    if (sum(valid_points) == 0) {
      return(list(
        summary = data.frame(
          ...
          sd_interval_width = NA,
          valid_test_points = 0,
          total_test_points = n_test
        ),
        individual_widths = rep(NA, n_test)
      ))
    }
    ...
    sd_width <- sd(interval_widths[valid_points], na.rm = TRUE)
    ...
    return(list(
      summary = data.frame(
        ...
        sd_interval_width = sd_width,
        valid_test_points = sum(valid_points),
        total_test_points = n_test
      ),
      individual_widths = interval_widths  # Return individual widths for each test point
    ))
  }


 
  # Compute intervals for all methods with individual widths (MODIFIED)
  methods <- c("uniLasso_loo_true", "uniLasso_loo_false", "polish_uniLasso", 
               "lasso_cv", "uniReg", "least_squares")
  pred_matrices <- list(pred_unilasso_loo_true, pred_unilasso_loo_false, 
                        pred_polish_unilasso, pred_lasso_cv, pred_unireg, pred_least_squares)

  intervals_summaries <- list()
  dataset_widths <- list()

  for (m in 1:length(methods)) {
    result <- compute_intervals_with_widths(pred_matrices[[m]], methods[m])
    intervals_summaries[[m]] <- result$summary
    dataset_widths[[methods[m]]] <- result$individual_widths
  }

  # Combine summaries
  intervals_this_dataset <- do.call(rbind, intervals_summaries)

  # Add to overall prediction intervals
  all_prediction_intervals <- rbind(all_prediction_intervals, intervals_this_dataset)

  # Store individual widths for this dataset (ADDED)
  all_interval_widths[[dataset_name]] <- dataset_widths
...
filename <- "unilasso_22datasets_100bootstrap_train50percent.csv"
...
write.csv(all_prediction_intervals, "unilasso_22datasets_100bootstrap_pred_int.csv", row.names = FALSE)
cat("Prediction interval results saved to: unilasso_22datasets_100bootstrap_pred_int.csv\n")

# Save individual interval widths (ADDED)
save(all_interval_widths, file = "unilasso_22datasets_100bootstrap_interval_widths.RData")
cat("Individual interval widths saved to: unilasso_22datasets_100bootstrap_interval_widths.RData\n")

# Alternative using saveRDS (ADDED):
# saveRDS(all_interval_widths, "unilasso_22datasets_100bootstrap_interval_widths.rds")
# cat("Individual interval widths saved to: unilasso_22datasets_100bootstrap_interval_widths.rds\n")