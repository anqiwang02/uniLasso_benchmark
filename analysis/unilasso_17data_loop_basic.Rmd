...
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript -e 'rmarkdown::render(\"unilasso_17data_loop_basic.Rmd\")'")
  quit("no")
}
...
# run below (see unilasso_17data_loop_basic) on stringsAsFactors
...
# updated 50-50 random split results with 100 different splits 9/3 wednesday for 17 datasets
...
# read csv file
results_all_splits <- read.csv("/accounts/grad/aqwang/unilasso/analysis/random_split_result/unilasso_17datasets_100splits_train50percent_update2_results.csv")
...
# updated 50-50 random split results with 100 different splits 9/3 wednesday for 22 datasets (choose one target for datasets with multiple targets )
```{r load results: comparing LS, uniReg, unilasso (loo=True) and polish unilasso with loo=TRUE with 50-50 train/test splits updated}
# read csv file
...
exclude_patterns <- c("communities_and_crime_unnormalized_\\(", "infrared_thermography_temperature_\\(",
                      "parkinsons_telemonitoring", "power_consumption_of_tetouan_city_\\(", "stock_portfolio_performance_\\(", "concrete_compressive_strength", "airfoil_self-noise", "combined_cycle_power_plant", "superconductivity_data" )

pat <- paste(exclude_patterns, collapse = "|")  # OR of substrings
keep <- !grepl(pat, results_all_splits$dataset) # TRUE = keep, FALSE = drop
results_all_splits <- results_all_splits[keep, ]

# print the unique names in results_all_splits$dataset to verify
print(unique(results_all_splits$dataset))

# 17 datasets left after excluding datasets with multiple targets and datasets that overlap with 17 datasets 
...
# calculate stability for all our 17 datasets
...
coef_csv_dir <- "/accounts/grad/aqwang/unilasso/analysis/coef_csv"
...
# delete rows where method="unilasso_loo_false"
plot_data <- plot_data[plot_data$method != "unilasso_loo_false", ]
#recode method names
...
# updated bootstrap results with uniReg for 17 datasets
```{r load updated bootstrap results and make some plots}

boot_results <- read.csv("/accounts/grad/aqwang/unilasso/analysis/bootstrap_result/unilasso_17datasets_100bootstrap_train50percent_all_results_parallel_new.csv")
...
plot_mse_data2 <- plot_mse_data %>%
  mutate(
    ...
    sd_label = case_when(
      sd_mse < 0.1 ~ formatC(mean_mse, format = "e", digits = 2),
...
# for prediction interval, make a plot of average width of prediction interval for each dataset with standard error bar
pred_int <- read.csv("/accounts/grad/aqwang/unilasso/analysis/bootstrap_result/unilasso_17datasets_100bootstrap_prediction_intervals_new.csv")
...
p_pred_int <- ggplot(pred_int2, aes(x = method, y = avg_interval_width, fill = method)) +
  ...
  geom_text(aes(label = label_text),  
            position = position_dodge(width = 0.6),
            vjust = -0.5, size = 3.3, angle = 80, hjust = -0.2) +
  # Facet with per-facet axes
  ggh4x::facet_wrap2(
    ~ dataset,
    nrow = ceiling(length(unique(pred_int$dataset))/5),
...
# updated bootstrap results with uniReg for 22 datasets
```{r load updated bootstrap results and make some plots}
boot_results <- read.csv("/accounts/grad/aqwang/unilasso/analysis/bootstrap_result/unilasso_22datasets_100bootstrap_train50percent_all_results_parallel.csv")

exclude_patterns <- c("communities_and_crime_unnormalized_\\(", "infrared_thermography_temperature_\\(",
                      "parkinsons_telemonitoring", "power_consumption_of_tetouan_city_\\(", "stock_portfolio_performance_\\(", "concrete_compressive_strength", "airfoil_self-noise", "combined_cycle_power_plant", "superconductivity_data" )

pat <- paste(exclude_patterns, collapse = "|")  # OR of substrings
keep <- !grepl(pat, boot_results$dataset) # TRUE = keep, FALSE = drop
boot_results <- boot_results[keep, ]
...
p_mse_error <- ggplot(plot_mse_data2, aes(x = method, y = mean_mse, fill = method)) +
  ...
  geom_text(aes(label = label_text),
            ...
            vjust = -2, size = 3.3, angle = 75, hjust = 0) +
  # Facet with per-facet axes
  ggh4x::facet_wrap2(
    ~ dataset,
    nrow = ceiling(length(unique(plot_mse_data$dataset))/5),
...
pat <- paste(exclude_patterns, collapse = "|")  # OR of substrings
keep <- !grepl(pat, pred_int$dataset) # TRUE = keep, FALSE = drop
pred_int <- pred_int[keep, ]
...
p_pred_int <- ggplot(pred_int2, aes(x = method, y = avg_interval_width, fill = method)) +
  ...
  geom_text(aes(label = label_text),  
            position = position_dodge(width = 1),
            vjust = -2, size = 3.3, angle = 80, hjust = 0) +
  # Facet with per-facet axes
  ggh4x::facet_wrap2(
    ~ dataset,
    nrow = ceiling(length(unique(pred_int$dataset))/5),
