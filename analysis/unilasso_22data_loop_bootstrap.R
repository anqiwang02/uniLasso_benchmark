# Loop to run uniLasso analysis on all 17 datasets with bootstrap sampling (PARALLELIZED)
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript unilasso_17data_loop_bootstrap.R")
  quit("no")
}
...
run_single_bootstrap <- function(bootstrap_id, Xtr_original, ytr_original, Xte, yte, dataset_name) {
  ...
  boot_results$unireg_support <- sum(coef(uniReg_cv, s = "lambda.min")[-1] != 0) # exclude intercept

  # Add OOB residuals
  if (length(oob_indices) > 0) {
    Xoob <- Xtr_original[oob_indices, , drop = FALSE]
    yoob <- ytr_original[oob_indices]
    yoob_pred <- predict(uniReg_cv, newx = Xoob, s = "lambda.min")
    oob_residuals <- yoob - as.vector(yoob_pred)
    sampled_residual <- sample(oob_residuals, size = length(yte), replace = TRUE)
    yte_pred_with_oob <- as.vector(yte_pred_CV) + sampled_residual
  } else {
...
for (i in seq_along(dataset_names)) {
  ...
  X <- read.csv(X_path)
  X <- as.matrix(X)
  y <- read.csv(y_path) # treat the first row as column name 
...
filename <- "unilasso_22datasets_100bootstrap_train50percent_all_results_parallel.csv"
...
write.csv(all_prediction_intervals, "unilasso_22datasets_100bootstrap_prediction_intervals.csv", row.names = FALSE)
cat("Prediction interval results saved to: unilasso_22datasets_100bootstrap_prediction_intervals.csv\n")
