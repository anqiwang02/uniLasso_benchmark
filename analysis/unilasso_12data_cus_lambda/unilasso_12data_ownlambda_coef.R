...
# Uses identical seeds to main analysis script (unilasso_12data_loop_basic.R) for exact reproducibility
...
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript unilasso_12data_ownlambda_coef.R")
  quit("no")
}
...
# Output dirs
coef_csv_dir <- "coef_csv_12datasets_ownlambda"
...
extract_coefficients_single_split <- function(split_id, X, y, dataset_name) {
  ...
  tryCatch({
    fit <- polish.uniLasso(
      ...
      family = "gaussian",
      foldid = foldid,
      nlambda = 100, #use the default here
      standardize = FALSE,
      intercept = TRUE,
      loo = TRUE
    )
  ...
  tryCatch({
    ...
    ##    1e-8 of lambda_max (much smaller than the default 1e-4)
  ...
  # 5) UniReg (CV) no regularization (lambda=0)
  tryCatch({
    fit <- cv.uniReg(
      Xtr, ytr,
      family = "gaussian",
    ...
    split_coeffs$unireg <- as.numeric(coef(fit, s =0)[-1])
  }, error = function(e) {
...
save(all_datasets_coefficients, file = "unilasso_12datasets_coefficients_only.RData")
...
cat("RData saved to:", normalizePath("unilasso_12datasets_coefficients_only.RData"), "\n")
...
cat('load("unilasso_12datasets_coefficients_only.RData")\n')
