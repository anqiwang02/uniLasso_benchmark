...
for (dataset in unique(df_long$Dataset)) {
  ...
  p <- ggplot(df_subset, aes(x = Col, y = Row, fill = Value)) +
    ...
    geom_text(aes(label = Value), color = "black", size = 5, face="bold", family="Times New Roman") +  # Reduced text size
    scale_fill_gradient(low = "white", high = "steelblue") +
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 16, face = "bold", family="Times New Roman"),  # Reduced text size
      axis.text.y = element_text(size = 16, face = "bold", family="Times New Roman"),
    ...
    labs(title = dataset,  # Shorter title
         x = NULL,  # Remove x-axis label to save space
         y = NULL,  # Remove y-axis label to save space
...
ggsave(
  filename = "similarity_cross_methods_results/heatmap_common_features_all_datasets.png", 
  plot = combined_plot, 
  width = 18,  # Increased width for 3 columns
  height = 22, # Increased height for 4 rows
  dpi = 300
)
