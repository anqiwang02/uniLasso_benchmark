...
plot_combined_long1_mse <- plot_combined_long1_mse %>% 
...
p_mse_ordered_normalized <- ggplot(plot_combined_long1_mse, aes(x = id_ordered, y = normalized_mse, color = method)) +
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/p_mse_line_12data9_normalized.png", 
       plot = p_mse_ordered_normalized, width = 18, height = 26, dpi = 300)
print(p_mse_ordered_normalized)
...
# drop all splid_id if one of them has value NA
plot_combined_long2_mse <- plot_combined_long2_mse %>% 
  group_by(dataset, split_id) %>% 
  filter(!any(is.na(value))) %>% 
  ungroup()
...
p_mse_ordered_normalized2 <- ggplot(plot_combined_long2_mse, aes(x = id_ordered, y = normalized_mse, color = method)) +
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/p_mse_line_12data3_normalized.png", 
       plot = p_mse_ordered_normalized2, width = 34, height = 30, dpi = 300)
print(p_mse_ordered_normalized2)
...
# choose three datasets: elevator, diamond, naval propulsion
...
# Apply max normalization within each dataset (divide by max)
plot_combined_long_mse <- plot_combined_long_mse %>%
  group_by(dataset) %>%
  mutate(
    max_mse = max(value, na.rm = TRUE),
    # Max normalization: x / max (makes max = 1, others proportional)
...
p_mse_ordered_normalized_nosign <- ggplot(plot_combined_long_mse, aes(x = id_ordered, y = normalized_mse, color = method)) +
  geom_point(alpha = 0.8, size = 4) +
  geom_line(alpha = 0.8, linewidth = 2) +
  facet_wrap(~ dataset, scales = "free") +  # Changed to "free" to allow both x and y axes to be free
  # Remove fixed y-axis limits since each panel will have its own scale
  ...
  theme(
    axis.text.x = element_text(angle = 40, hjust = 1, size = 50, family = "Times New Roman", face = "bold"),
    axis.text.y = element_text(size = 50, family = "Times New Roman", face = "bold"),  
    axis.title = element_text(size = 70, family = "Times New Roman", face = "bold"),
    legend.position = "bottom",
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/p_mse_line_12data3_nosign_normalized.png", 
       plot = p_mse_ordered_normalized_nosign, width = 40, height = 40, dpi = 300)
print(p_mse_ordered_normalized_nosign)


# Support metric for the last 3 datasets
...
# Save output
ggsave(
  "/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/p_support_bar_12datalast3_nosign.png", 
  plot = p_support_bar2,
  width = 10,
  height = 8,
  dpi = 300
...
# Add normalized MSE for each dataset
plot_combined_long_normalized<- plot_combined_long %>%
  filter(metric == "MSE") %>%
  group_by(dataset) %>%
  mutate(
    max_mse = max(value, na.rm = TRUE),
    # Max normalization: x / max (makes max = 1, others proportional)
    normalized_value = ifelse(max_mse == 0, 0, value / max_mse)
  ) %>%
  ungroup() %>%
  mutate(
  ...
  select(-max_mse, -normalized_value)

# Combine original data with normalized MSE
plot_combined_long_with_normalized <- bind_rows(
  plot_combined_long,
  plot_combined_long_normalized
) %>% filter(metric != "MSE")  %>% # Remove original MSE rows
  mutate(
    metric = factor(metric, levels = c("Normalized test MSE", "Support"))
  )
...
plot_combined_long3 <- plot_combined_long_with_normalized %>% filter(dataset %in% c("Elevator", "Diamond", "Naval propulsion")) 
...
# 3. Plot with error bars but no text labels - wider version
plot_bootstrap_points_sd <- ggplot(plot_combined_long3, aes(x = method, y = value, color = method)) +
  geom_jitter(width = 0.3, alpha = 0.4, size = 0.8) +  # Increased jitter width from 0.15 to 0.3
  geom_errorbar(
    data = plot_bootstrap_summary,
    aes(x = method, ymin = lower, ymax = upper),
    width = 0.4,  # Increased error bar width from 0.25 to 0.4
    linewidth = 0.8,
    color = "black",
    inherit.aes = FALSE
  ) +
  ...
  scale_color_manual(values = method_colors) +
  # Add horizontal expansion to spread out the x-axis
  scale_x_discrete(expand = expansion(add = 0.8)) +  # Add space around x-axis categories
  ...
  theme(
    ...
    panel.spacing.x = unit(1.2, "cm"),  # Increase horizontal spacing between panels
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
  )
...
# Increase plot width significantly
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/p_mse_supp_bootstrap_3data.png", 
       plot = plot_bootstrap_points_sd, width = 15, height = 18, dpi = 300)  # Increased width from 
...
# choose first 5 
plot_combined_long5 <- plot_combined_long_with_normalized %>% filter(dataset %in% c("CA housing", "Debutanizer", "Insurance", "Kin8nm", "Computer")) 

# Compute mean and ±1 SD for each dataset × metric × method
plot_bootstrap_summary5 <- plot_combined_long5 %>%
  group_by(dataset, metric, method) %>%
  summarise(
    mean_value = mean(value, na.rm = TRUE),
    sd_value   = sd(value, na.rm = TRUE),
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/app_mse_supp_bootstrap_5data.png", 
       plot = plot_bootstrap_points_sd5, width = 15, height = 18, dpi = 300)  # Increased dimensions for 9 datasets




# choose another4 
plot_combined_long4 <- plot_combined_long_with_normalized %>% filter(dataset %in% c( "Energy efficiency", "Miami housing", "Protein structure", "QSAR")) 

# Compute mean and ±1 SD for each dataset × metric × method
plot_bootstrap_summary4 <- plot_combined_long4 %>%
  group_by(dataset, metric, method) %>%
  summarise(
    mean_value = mean(value, na.rm = TRUE),
    sd_value   = sd(value, na.rm = TRUE),
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/app_mse_supp_bootstrap_4data.png", 
       plot = plot_bootstrap_points_sd4, width = 15, height = 18, dpi = 300)  # Increased dimensions for 9 datasets



################################mean and sd of normalized MSE and Support #############################
# Compute mean and ±1 SD for each dataset × metric × method;
plot_bootstrap_summary <- plot_combined_long_with_normalized %>%
  group_by(dataset, metric, method) %>%
  summarise(
    mean_value = mean(value, na.rm = TRUE),
    sd_value   = sd(value, na.rm = TRUE),
  ...
  mutate(
    ...
    mean_formatted = ifelse(abs(mean_value) >= 10000 | abs(mean_value) < 0.09,
                           formatC(mean_value, format = "e", digits = 2),
                           formatC(mean_value, format = "f", digits = 2)),
    
    sd_formatted = ifelse(abs(sd_value) >= 10000 | abs(sd_value) < 0.09,
                         formatC(sd_value, format = "e", digits = 2),
                         formatC(sd_value, format = "f", digits = 2))
  ) %>%
mutate(label_text = paste0(mean_formatted, " ± ", sd_formatted)) %>%
select(dataset, metric, method, label_text) 
view(plot_bootstrap_summary)

# select for MSE metric only and make a wide format
plot_bootstrap_summary_MSE <- plot_bootstrap_summary %>% filter(metric == "Normalized test MSE") %>% pivot_wider(names_from = method, values_from = label_text)
View(plot_bootstrap_summary_MSE)
#save as csv
write.csv(plot_bootstrap_summary_MSE, "/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/boot_summary_normalized_MSE_3datasets.csv", row.names = FALSE)

# select for support 
plot_bootstrap_summary_support <- plot_bootstrap_summary %>% filter(metric == "Support") %>% pivot_wider(names_from = method, values_from = label_text)
View(plot_bootstrap_summary_support)
#save as csv
write.csv(plot_bootstrap_summary_support, "/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/boot_summary_support_3datasets.csv", row.names = FALSE)
...
# Plot prediction interval widths with error bars but no text labels
plot_pred_int_width_sd <- ggplot(pred_int_width3, aes(x = method, y = width, color = method)) +
  geom_jitter(width = 0.25, alpha = 0.2, size = 0.5) +  # Increased width, reduced alpha and size
  geom_errorbar(
  ...
  scale_color_manual(values = method_colors) +
  scale_x_discrete(expand = expansion(add = 0.8)) +  # Add more space around x-axis categories
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.2))) +
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/pred_int_width_bootstrap_3data.png", 
       plot = plot_pred_int_width_sd, width = 16, height = 8, dpi = 300)  # Increased width from 12 to 16
...
# Compute mean and ±1 SD for each dataset × method for prediction interval width
pred_int_width_summary9 <- pred_int_width9 %>%
  group_by(dataset, method) %>%
  summarise(
    mean_value = mean(width, na.rm = TRUE),
    sd_value   = sd(width, na.rm = TRUE),
...
ggsave("/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/pred_int_width_bootstrap_9data.png", 
       plot = plot_pred_int_width_sd9, width = 18, height = 24, dpi = 300)  # Increased width from 15 to 18


############# Compute mean and ±1 SD for each dataset × method for prediction interval width
pred_int_width_summary <- pred_int_width %>%
  group_by(dataset, method) %>%
  summarise(
    mean_value = mean(width, na.rm = TRUE),
    sd_value   = sd(width, na.rm = TRUE),
...
view(pred_int_width_summary)

# Make a wide format for prediction interval width
pred_int_width_summary_wide <- pred_int_width_summary %>% 
  pivot_wider(names_from = method, values_from = label_text)

View(pred_int_width_summary_wide)

# Save as csv
write.csv(pred_int_width_summary_wide, "/accounts/grad/aqwang/unilasso/figures_12data/figure_default_lambda/pred_int_width_summary_12datasets.csv", row.names = FALSE)
