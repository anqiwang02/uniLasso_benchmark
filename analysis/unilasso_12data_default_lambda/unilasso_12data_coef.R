...
if (R.home() != "/scratch/users/aqwang/conda/envs/r_package/lib/R") {
  system("/scratch/users/aqwang/conda/envs/r_package/bin/Rscript unilasso_12data_coef.R")
  quit("no")
}
...
# Output dirs
coef_csv_dir <- "coef_csv_12datasets_default_lambda"
...
extract_coefficients_single_split <- function(split_id, X, y, dataset_name) {
  ...
  tryCatch({
    ...
    split_coeffs$unireg <- as.numeric(coef(fit, s = 0)[-1]) #regulatization parameter 0
  }, error = function(e) {
...
# save the list to Rdata 
save(all_datasets_coefficients, file = "unilasso_12datasets_all_coefficients.RData")
